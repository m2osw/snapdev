 ...
 *
 * \todo
 * The read-only feature does not work because the std implementation
 * writes to our buffer on its own so we are not told that a write
 * happens.
 *
 * \todo
 ...
 *
 * \todo
 * Implement the seekpos() & Co. so the tellg() and tellp() work as
 * expected (i.e. the base class does not provide those).
 *
